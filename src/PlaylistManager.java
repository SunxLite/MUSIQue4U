 ...
 */
   ...
   // display what a specified playlist contains
       static void display(Playlist playlist) {
         System.out.println("Data of " + playlist);
   ...
   // display the details of a specified array of media objects
       static void display(Media[] media) {
         ...
         if (media != null) {
            ...
            System.out.println("Playlist currently contain no media.");
         }
         System.out.println(); // formating
      }
		...
		try{
         for (int i = 0; i < given.length; i++) {
            for (int a = 0; a < find.length; a++) {
               if (find[a] instanceof Music && given[i] instanceof Music){
                  if (((Music)given[i]).equals((Music)find[a])) { // When it matches what the user specified
                     found.addMedia(given[i]);
                  }
               }
               else if(find[a] instanceof Video && given[i] instanceof Video){
                  if (((Video)given[i]).equals((Video)find[a])) {
                     found.addMedia(given[i]);
                  }
               }
            }
   ...
   // ============================== SORT ==============================
      ...
      // Now all media sort are alike..
         quickSort(items, 0, items.length-1, req);
         return items;
      }
      ...
      // An array of styles allow variety
         ...
         // Check sort style, since there is no eval functions in Java...
            if (style[0].equals("") && style[1].equalsIgnoreCase("Title")) {
               while (x != y) {
            ...
            else if (style[0].equals("") && style[1].equalsIgnoreCase("Genre")) {
               while (x != y) {
